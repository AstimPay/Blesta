...
class Astimpay extends NonmerchantGateway
{
    private $meta;

    public function __construct()
    {
        Loader::load(dirname(__FILE__) . DS . 'lib' . DS . 'AstimPayAPI.php');

        $this->loadConfig(dirname(__FILE__) . DS . 'config.json');

        Loader::loadComponents($this, ['Input']);

        Language::loadLang('astimpay', null, dirname(__FILE__) . DS . 'language' . DS);
    }
    ...
    public function getSettings(array $meta = null)
    {
        ...
        $this->view->setDefaultView(
            'components' . DS . 'gateways' . DS . 'nonmerchant' . DS . 'astimpay' . DS
        );
        ...
        $rules = [
            'api_key'       => [
                'valid' => [
                    'rule'    => 'isEmpty',
                    'negate'  => true,
                    'message' => Language::_('AstimPay.!error.api_key.valid', true),
                ],
            ],
            'api_url'       => [
                'valid' => [
                    'rule'    => 'isEmpty',
                    'negate'  => true,
                    'message' => Language::_('AstimPay.!error.api_url.valid', true),
                ],
            ],
            'exchange_rate' => [
                'valid' => [
                    'rule'    => 'isEmpty',
                    'negate'  => true,
                    'message' => Language::_('AstimPay.!error.exchange_rate.valid', true),
                ],
            ],
        ];
    ...
    public function buildProcess(array $contact_info, $amount, array $invoice_amounts = null, array $options = null)
    {
        ...
        . Configure::get('Blesta.company_id') . '/astimpay/?client_id='
    ...
    {
        return new AstimPayAPI($this->meta['api_key'], $this->meta['api_url']);
    }
